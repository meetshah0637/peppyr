import React from 'react';

interface LogoProps {
  width?: number | string;
  height?: number | string;
  showText?: boolean;
  textPosition?: 'right' | 'bottom';
  className?: string;
  variant?: 'full' | 'icon' | 'compact';
}

export const Logo: React.FC<LogoProps> = ({
  width = 40,
  height = 40,
  showText = true,
  textPosition = 'right',
  className = '',
  variant = 'full',
}) => {
  const isCompact = variant === 'compact';
  const logoSize = isCompact ? 32 : typeof width === 'number' ? width : 40;

  return (
    <div
      className={`inline-flex ${textPosition === 'bottom' ? 'flex-col items-center' : 'items-center'} ${className}`}
      style={{ gap: isCompact ? '0.375rem' : '0.5rem' }}
    >
      <svg
        width={width}
        height={height}
        viewBox="0 0 120 120"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="flex-shrink-0"
      >
        <defs>
          <linearGradient id="bubbleGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style={{ stopColor: '#6366f1', stopOpacity: 1 }} />
            <stop offset="100%" style={{ stopColor: '#8b5cf6', stopOpacity: 1 }} />
          </linearGradient>
          <linearGradient id="accentGradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style={{ stopColor: '#f59e0b', stopOpacity: 1 }} />
            <stop offset="100%" style={{ stopColor: '#ef4444', stopOpacity: 1 }} />
          </linearGradient>
        </defs>
        
        {/* Main speech bubble */}
        <path
          d="M95 15H25C20.0294 15 16 19.0294 16 24V74C16 78.9706 20.0294 83 25 83H45L60 100L75 83H95C99.9706 83 104 78.9706 104 74V24C104 19.0294 99.9706 15 95 15Z"
          fill="url(#bubbleGradient)"
          rx="8"
        />
        
        {/* Accent dots representing messages/templates */}
        <circle cx="35" cy="45" r="6" fill="url(#accentGradient)"/>
        <circle cx="60" cy="45" r="6" fill="url(#accentGradient)"/>
        <circle cx="85" cy="45" r="6" fill="url(#accentGradient)"/>
        
        {/* Text lines representing message content */}
        <rect x="30" y="58" width="50" height="4" rx="2" fill="white" opacity="0.9"/>
        <rect x="30" y="66" width="40" height="4" rx="2" fill="white" opacity="0.7"/>
      </svg>
      
      {showText && (
        <span
          className={`font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent ${
            textPosition === 'bottom' ? 'text-center' : ''
          }`}
          style={{
            fontSize: isCompact
              ? '1rem'
              : typeof width === 'number'
              ? `${Math.max(width * 0.4, 16)}px`
              : '1.25rem',
            lineHeight: '1.2',
          }}
        >
          peppyr
        </span>
      )}
    </div>
  );
};

export default Logo;
