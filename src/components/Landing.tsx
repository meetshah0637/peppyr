...
export const Landing: React.FC<LandingProps> = ({ defaultMode = 'signin' }) => {
  ...
  const handleEmail = async () => {
    ...
    try {
      if (mode === 'signin') {
        ...
        // Signup successful - onAuthStateChanged will handle user verification
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
        ...
        {!isFirebaseConfigured() && (
          <div className="text-center p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
            Firebase is not configured. The app will work in local storage mode. To enable authentication, please configure Firebase environment variables.
          </div>
        )}
